...
def vis_one_image(
        ...
        ext='png', out_when_no_box=False, return_kp=False, return_box=False):
    ...
    for i in sorted_inds:
        ...
        if keypoints is not None and len(keypoints) > i:
            ...
            # plot head even for small threshold
            if kps[2, 0] > kp_thresh:
                plt.plot(
                    kps[0, 0], kps[1, 0], '.', color='r',
    ...
    if return_kp:
        if keypoints is not None:
            out_kp = []
            for kp in keypoints:
                out_kp.append(kp[:3, [0,5,6,7,8,9,10]])
    if return_box:
        out_kp = (out_kp, boxes)
    return out_kp
