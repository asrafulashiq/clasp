...
def plot_latent_feature(x_test, encoded_imgs, decoded_imgs, img_y, img_x, names):
    ...
    # plt.show()
    plt.savefig(
        '/media/siddique/Data/CLASP2018/img/MOT/MOT17Det/train/MOT17-09/latent_feature/' + names[-10:],
        dpi=300)
    plt.close()
...
def cluster_mode_det(fr,latent_feature, labels,cluster_center, det_frame,
                     n_angle,score_th,ID_ind,min_cluster_size,iou_thr):
    ...
    final_det, det_frame, ID_ind, cluster_prob_score = cluster_association(fr, latent_feature, det_frame,
                                                                   labels, cluster_center, ID_ind,
                                                                    n_angle,score_th, min_cluster_size,iou_thr)
...
def plot_mask_cluster(x_test, labels,max_instances_at_theta,img_y,img_x, names,path):
    ...
    for i in range(ids.shape[0]):
        ...
        if (i == 1):
            for j in range(counts[i]):
                ax = plt.subplot(max_instances_at_theta, counts.max(), counts.max() + j + 1)
                plt.imshow(cluster_i[j].reshape(img_y, img_x), cmap='hot')
                # plt.gray()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
        if (i == 2):
            for j in range(counts[i]):
                ax = plt.subplot(max_instances_at_theta, counts.max(), 2 * counts.max() + j + 1)
                plt.imshow(cluster_i[j].reshape(img_y, img_x), cmap='hot')
                # plt.gray()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
        if (i == 3):
            for j in range(counts[i]):
                ax = plt.subplot(max_instances_at_theta, counts.max(), 3 * counts.max() + j + 1)
                plt.imshow(cluster_i[j].reshape(img_y, img_x), cmap='hot')
                # plt.gray()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
        if (i == 4):
            for j in range(counts[i]):
                ax = plt.subplot(max_instances_at_theta, counts.max(), 4 * counts.max()+j + 1)
                plt.imshow(cluster_i[j].reshape(img_y, img_x), cmap='hot')
                # plt.gray()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
        if (i == 5):
            for j in range(counts[i]):
                ax = plt.subplot(max_instances_at_theta, counts.max(), 5*counts.max() + j + 1)
                plt.imshow(cluster_i[j].reshape(img_y, img_x), cmap='hot')
                # plt.gray()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
        if (i == 6):
            for j in range(counts[i]):
                ax = plt.subplot(max_instances_at_theta, counts.max(), 6 * counts.max() + j + 1)
                plt.imshow(cluster_i[j].reshape(img_y, img_x), cmap='hot')
                # plt.gray()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
        if (i == 7):
            for j in range(counts[i]):
                ax = plt.subplot(max_instances_at_theta, counts.max(), 7 * counts.max() + j + 1)
                plt.imshow(cluster_i[j].reshape(img_y, img_x), cmap='hot')
                # plt.gray()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
        if (i == 8):
            for j in range(counts[i]):
                ax = plt.subplot(max_instances_at_theta, counts.max(), 8 * counts.max() + j + 1)
                plt.imshow(cluster_i[j].reshape(img_y, img_x), cmap='hot')
                # plt.gray()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
...
def tracking_temporal_clustering(fr,pax_boxs,time_lag,
                                 min_cluster_size,iou_thr,
                                 det_cluster_id,ID_ind,score_th,ax,im_h, im_w):
    #-----------------------------------------------------------------------------
    # **fr - current frame
    ...
    for i in np.linspace(fr-time_lag+1, fr, num=time_lag):
        ...
        if len(det_cluster_id)>0 and i<fr: #(fr=6, i=2,3,4,5 has already cluster id initialized detections)
    ...
    if temp_window_pbox is not None:
        ...
        cluster_center,labels,inertia = k_means(latent_feature,n_clusters= max_instances_at_theta)#,\
        ...
        refined_det,det_cluster_id,ID_ind,cluster_prob_score = cluster_mode_det(fr,latent_feature, labels,
                                                                                cluster_center,
                                                                                temp_pax_boxs,time_lag,score_th,
                                                                                ID_ind,min_cluster_size,iou_thr)
        ...
        for i in range(len(refined_det)):
            ...
            if (cluster_prob_score >= score_th and sum(refined_det[i][2:5])>0):# and refined_det[i, 8]>0
                refined_det[i, 0]=fr
                pax_eval.append(refined_det[i])
                color_mask = color[int(refined_det[i, 8])]#np.array([0, 1, 0], dtype='float32')  # green
                box_color = color[int(refined_det[i, 8])]
                score_text = str(int(refined_det[i, 8])) + '  {:0.2f}'.format(cluster_prob_score)
                ax = box_mask_overlay(refined_det[i], ax, im_h, im_w, score_text,color_mask,box_color)
...
for names in files:
    ...
    if (len(fr_pax_box)>0):
        ...
        if (fr >= pax_boxs_prev[:,0][0] + time_lag-1):
            fig, pax_track, ax, det_cluster_id, ID_ind = tracking_temporal_clustering(fr, pax_boxs_prev, time_lag,
                                                                                      min_cluster_size, iou_thr,
                                                                                      det_cluster_id,ID_ind,
                                                                                      score_th, ax, im_h, im_w)
            print('Total tracked ID:', ID_ind)
            pax_eval.append(pax_track)
            fig.savefig('/media/siddique/CLASP2019/eval_track/visualize/exp10A/' + names[-10:],dpi=dpi)
        plt.close('all')
    fr += 1

pax_track = np.array(pax_eval)
np.savetxt('/media/siddique/CLASP2019/eval_track/exp10A_track', pax_eval, fmt='%.4e',
           delimiter=',')
