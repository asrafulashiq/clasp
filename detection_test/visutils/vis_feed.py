...
class ImModule():
    ...
    def draw(self, im, text=""):
        ...
        ((txt_w, txt_h), _) = cv2.getTextSize(text, self.font, self.fontScale,
                                              self.fontThick)
...
class FeedModule:
    def __init__(self, width, height, padding=0.03, max_msg=10):
        ...
        for i in range(max_msg):
            ...
            self.msg_bounds.append([int(i) for i in (x1, y1, x2, y2)])
    ...
    def drawText(self, msg_list):
        ...
        ((_, txt_h), _) = cv2.getTextSize(self.header, self.font,
                                          self.fontScale, self.fontThick)
...
class VisFeed():
    def __init__(self, width=1600, height=900, padding=0.02):
        ...
        self.im_mod_1 = ImModule(w_,
                                 h_,
                                 pre_text="Camera 9 : ",
                                 padding=padding)

        # mod 2
        self.im_mod_2 = ImModule(w_,
                                 h_,
                                 pre_text="Camera 11 : ",
                                 padding=padding)

        # mod3
        self.im_mod_3 = ImModule(w_,
                                 h_,
                                 pre_text="Camera 13 : ",
                                 padding=padding)
    ...
    def draw(self, im1, im2, im3, frame_num, msglist, with_feed=True):
        ...
        if im1 is not None:
            can1 = self.im_mod_1.draw(im1,
                                      text=str(to_sec(frame_num)).zfill(4) +
                                      ' sec')
        if im2 is not None:
            can2 = self.im_mod_2.draw(im2,
                                      text=str(to_sec(frame_num)).zfill(4) +
                                      ' sec')
        if im3 is not None:
            can3 = self.im_mod_3.draw(im3,
                                      text=str(to_sec(frame_num)).zfill(4) +
                                      ' sec')
