...
parser = argparse.ArgumentParser(
    description='PyTorch Tracking SiamMask Training')
...
def train(train_loader, model, optimizer, lr_scheduler, epoch, cfg):
    ...
    for iter, input in enumerate(train_loader):
        ...
        x = {
            ...
            'label_mask':
            torch.autograd.Variable(input[6]).cuda(non_blocking=True),
            'label_mask_weight':
            torch.autograd.Variable(input[7]).cuda(non_blocking=True),
        }
