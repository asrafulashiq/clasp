...
parser = argparse.ArgumentParser(
    description='Finetune parameters for SiamMask tracker on VOT')
...
parser.add_argument('-l',
                    '--log',
                    default="log_tune.txt",
                    type=str,
                    help='log file')
parser.add_argument('--penalty-k',
                    default='0.05,0.5,0.05',
                    type=parse_range,
                    help='penalty_k range')
parser.add_argument('--lr',
                    default='0.35,0.5,0.05',
                    type=parse_range,
                    help='lr range')
...
def tune(param):
    ...
    tracker_path = join(
        ...
        (param['network_name'] + '_r{}'.format(param['hp']['instance_size']) +
    ...
    elif param['dataset'].startswith('OTB') or param['dataset'].startswith(
            'DAVIS'):
    ...
    for f, im in enumerate(ims):
        ...
        if f == start_frame:  # init
            ...
            if param['dataset'].startswith('VOT'):
                regions.append(1)
            elif param['dataset'].startswith(
                    'OTB') or param['dataset'].startswith('DAVIS'):
                regions.append(gt[f])
    ...
    print('Video: {:12s} Time: {:2.1f}s Speed: {:3.1f}fps Lost: {:d}'.format(
        param['video'], toc, f / toc, lost_times))
...
def main():
    ...
    p['network'] = model
    p['network_name'] = args.arch + '_' + args.resume.split('/')[-1].split(
        '.')[0]
