...
def main():
    ...
    dist_model = torch.nn.DataParallel(
        model, list(range(torch.cuda.device_count()))).cuda(non_blocking=True)
    ...
    if args.resume:
        ...
        dist_model = torch.nn.DataParallel(
            model,
            list(range(torch.cuda.device_count()))).cuda(non_blocking=True)
...
def train(train_loader, model, optimizer, lr_scheduler, epoch, cfg):
    ...
    for iter, input in enumerate(train_loader):
        ...
        x = {
            ...
            cfg,
            'template':
            torch.autograd.Variable(input[0]).cuda(non_blocking=True),
            'search':
            torch.autograd.Variable(input[1]).cuda(non_blocking=True),
            'label_cls':
            torch.autograd.Variable(input[2]).cuda(non_blocking=True),
            'label_loc':
            torch.autograd.Variable(input[3]).cuda(non_blocking=True),
            'label_loc_weight':
            torch.autograd.Variable(input[4]).cuda(non_blocking=True),
        }
        ...
        rpn_cls_loss, rpn_loc_loss = torch.mean(rpn_cls_loss), torch.mean(
            rpn_loc_loss)
        ...
        if (iter + 1) % args.print_freq == 0:
            logger.info(
                ...
                '\t{rpn_cls_loss:s}\t{rpn_loc_loss:s}\t{siamrpn_loss:s}'.
