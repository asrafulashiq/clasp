...
if "-ioupax" in sys.argv:
	pax_intersectionOverUnionThreshold = sys.argv[sys.argv.index("-ioupax") + 1]
else:
	pax_intersectionOverUnionThreshold = .3

if "-ioudvi" in sys.argv:
        dvi_intersectionOverUnionThreshold = sys.argv[sys.argv.index("-ioudvi") + 1]
else:
        dvi_intersectionOverUnionThreshold = .2
...
if "-m" in sys.argv:
	...
	for entry in sys.argv[sys.argv.index("-m") + 1:]:
		...
		try:
			...
			cam = entry[0:entry.index("exp")].replace("cam", '').replace("./", '')
			camera.append(cam)
		except:
			break;
	i = 0;
...
if "-g" in sys.argv:
	...
	try:
		gt_in = "gt/cam09exp2-Training-logfile.txt"
		gt_output = open(gt_in, 'r').read().split('\n')
	except:
...
def annotateBB(frame, entry, t, iou):
	frame = int(frame) + 1;
	f = "cam" + str(entry[entry.index("camera-num") + 1]) + experiment + "/" + str(frame).zfill(5) + ".jpg"
	img = cv2.imread(f);
	...
	if t == "ata":
		...
		fontColor = (0,0,255)
		cv2.putText(img, str(entry[entry.index("id") + 1]), bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
		...
		cv2.putText(img, "{0:.2f}".format(iou), bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
	elif t == "gt":
		...
		try:
			cv2.putText(img, str(getFrameObjects(entry[entry.index("id") + 1])), bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
		except KeyError:
			cv2.putText(img, str(entry[entry.index("id") + 1]), bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
	elif t == "fa":
...
def annotateText(frame, entry, t, ataFrame):
	...
	if t == "gt":
		bottomLeftCornerOfText = (10,50)
       		fontColor = (0,255,0)
		cv2.putText(img,'GT  XFR EVENT', bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
	elif t == "ata":
		bottomLeftCornerOfText = (10,100)
                fontColor = (0,0,255)
		
		if int(ataFrame) + 1 - int(frame) < 0:
			cv2.putText(img,"ATA XFR EVENT at " + str(int(ataFrame) + 1 - int(frame)) + " frames", bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
		elif int(ataFrame) + 1 - int(frame) > 0:
			cv2.putText(img,"ATA XFR EVENT at +" + str(int(ataFrame) + 1 - int(frame)) + " frames", bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
		else:
			cv2.putText(img,"ATA XFR EVENT +/- 0 frames", bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
	elif t =="fa":
		bottomLeftCornerOfText = (10,100)
                fontColor = (0,255,255)

		if int(ataFrame) + 1 - int(frame) < 0:
                        cv2.putText(img,"ATA XFR EVENT False Alarm at " + str(int(ataFrame) + 1 - int(frame)) + " frames", bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
                elif int(ataFrame) + 1 - int(frame) > 0:
                        cv2.putText(img,"ATA XFR EVENT False Alarm at +" + str(int(ataFrame) + 1 - int(frame)) + " frames", bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
                else:
                        cv2.putText(img,"ATA XFR EVENT False Alarm at +/- 0 frames", bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
	else:
		bottomLeftCornerOfText = (10,150)
                fontColor = (255,255,255)
		cv2.putText(img, ataFrame, bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
...
def formatHeader():
	...
	# HEADER
	try:
                ret_string = ret_string +  "# ##### HEADER for " + str(experiment) + " #####\n"
        except NameError:
		ret_string = ret_string + "# ##### HEADER #####\n"
	# Date & time
        ret_string = ret_string + "# time: " + str(cmd("date +%Y-%m-%d\" \"%H:%M:%S")[1]) + "\n"
	...
	ret_string = ret_string + "# total frames: " + str(totalFrames) + "\n"
        ...
        ret_string = ret_string + "# total records: " + str(int(results["# PAX total"] + results["# DVI total"] + results["# XFR total"])) + "\n"

	# total events by type
	ret_string = ret_string + "# total GT PAX location events: " + str(results["# PAX total"]) + "\n"
	ret_string = ret_string + "# total GT DVI location events: " + str(results["# DVI total"]) + "\n"
	ret_string = ret_string + "# total GT XFR events: " + str(results["# XFR total"]) + "\n"

        # which cameras?
        ret_string = ret_string + "# cameras: " + str(camera).replace('[', '').replace('\'', '').replace(']', '') + "\n"
        # iou threshold
	ret_string = ret_string + "# PAX IoU threshold: " + str(pax_intersectionOverUnionThreshold) + "\n"
	ret_string = ret_string + "# DVI IoU threshold: " + str(dvi_intersectionOverUnionThreshold) + "\n"

	# xfr thresdhold
	ret_string = ret_string + "# XFR threshold: " + str(xfrThreshold) + "\n"
	# version of scoring tool
        ret_string = ret_string + "# scoring Tool Version: " + str(scoringToolVersion) + "\n\n"
...
def formatScoreReport(results, gtDebug, ataDebug):
	...
	# RESULTS
	try:
		scoreCard.write("PD (PAX) = %" + "{0:.1f}".format(float((float(results["# PD (PAX)"]))/results["# PAX total"])*100) + " (" + str(results["# PD (PAX)"]) + "/" + str(results["# PAX total"]) +")\n");
	except ZeroDivisionError:
		scoreCard.write("PD (PAX) = N/A\n");
	try:
		scoreCard.write("PD (DVI) = %" + "{0:.1f}".format(float((float(results["# PD (DVI)"]))/results["# DVI total"])*100) + " (" + str(results["# PD (DVI)"]) + "/" + str(results["# DVI total"]) +")\n");
	except ZeroDivisionError:
		scoreCard.write("PD (DVI) = N/A\n");
	try:
		scoreCard.write("PD (XFR) = %" + "{0:.1f}".format(float((float(results["# PD (XFR)"]))/results["# XFR total"])*100) + " (" + str(results["# PD (XFR)"]) + "/" + str(results["# XFR total"]) +")\n");
	except ZeroDivisionError:
		scoreCard.write("PD (XFR) = N/A\n");

	try:
		scoreCard.write("PFA (PAX) = %" + "{0:.1f}".format(float((float(results["# PFA (PAX)"]))/round(totalFrames*len(camera)/100))*100) + " (" + str(results["# PFA (PAX)"]) + "/" + str(int(round(totalFrames*len(camera)/100.0))) +")\n");
	except ZeroDivisionError:
		scoreCard.write("PFA (PAX) = N/A\n");
	
	try:
		scoreCard.write("PFA (DVI) = %" + "{0:.1f}".format(float((float(results["# PFA (DVI)"]))/round(totalFrames*len(camera)/100))*100) + " (" + str(results["# PFA (DVI)"]) + "/" + str(int(round(totalFrames*len(camera)/100.0))) +")\n");
	except ZeroDivisionError:
		scoreCard.write("PFA (DVI) = N/A\n");
	try:
		scoreCard.write("PFA (XFR) = %" + "{0:.1f}".format(float((float(results["# PFA (XFR)"]))/round(totalFrames*len(camera)))*100) + " (" + str(results["# PFA (XFR)"]) + "/" + str(int(round(totalFrames*len(camera)))) +")\n");
	except ZeroDivisionError:
		scoreCard.write("PFA (XFR) = N/A\n")

	try:
		scoreCard.write("P (PAX switch) = %" + "{0:.1f}".format(float(float(results["# pax switch"])/float(results["# PAX total"]))*100) + " (" + str(results["# pax switch"]) + "/" + str(results["# PAX total"]) +")\n");
	except ZeroDivisionError:
		scoreCard.write("P (PAX switch) = N/A\n")
	try:
		scoreCard.write("P (DVI switch) = %" + "{0:.1f}".format(float(float(results["# dvi switch"])/float(results["# DVI total"]))*100) + " (" + str(results["# dvi switch"]) + "/" + str(results["# DVI total"]) +")\n");
	except ZeroDivisionError:
		scoreCard.write("P (DVI switch) = N/A\n")
		
	try:
		scoreCard.write("P (mismatch) = %" + "{0:.1f}".format(float(float(results["# mismatch"])/float(results["# XFR total"]))*100) + " (" + str(results["# mismatch"]) + "/" + str(results["# XFR total"]) +")\n");
	except ZeroDivisionError:
...
def formatData(data):
	...
	for d in data:
		entry = d.lower().replace(":", '').replace("\r", '').replace(',','').split(" ")
		if entry[0] == "#" and entry[1] == "frame":
...
def IoU(r1, r2):
        ...
        rect1 = { "x" : float(r1[0]), "width" : float(r1[2]) - float(r1[0]), "y" : float(r1[1]), "height" : float(r1[3]) - float(r1[1])}
        rect2 = { "x" : float(r2[0]), "width" : float(r2[2]) - float(r2[0]), "y" : float(r2[1]), "height" : float(r2[3]) - float(r2[1])}
...
def match_loc_event(ata, event, frame, t):
	...
	for entry in ata[frame]:
		...
		if entry[entry.index("camera-num") + 1] != event[event.index("camera-num") + 1]:
			continue

		iou = IoU(entry[entry.index("bb") + 1 : entry.index("bb") + 5], event[event.index("bb") + 1 : event.index("bb") + 5]);
		...
		if float(iou) > float(dvi_intersectionOverUnionThreshold) and entry[entry.index("type") + 1] == "dvi" or \
			...
			try:
                                ...
                                if getFrameObjects(event[event.index("id") + 1]) != entry[entry.index("id") + 1]:
                                        setFrameObjects(event[event.index("id") + 1], entry[entry.index("id") + 1]);
			...
			# NOTE when the tool matches multiple log entries in the ata to a single gt location event
                	...
                	if entry[entry.index("id") + 1] in obs.keys():
                        	if obs[entry[entry.index("id") + 1]]["entry"][3] == entry[entry.index("type") + 1] and obs[entry[entry.index("id") + 1]]["camera-num"] == entry[entry.index("camera-num") + 1]:
                                	#if obs[entry[entry.index("id") + 1]]["entry"] != entry:
                                	print "reused ID at frame " + str(frame) + ": " + str(entry[entry.index("id") + 1])
                        	        WARNING = True;
                	obs[entry[entry.index("id") + 1]] = { "camera-num" : entry[entry.index("camera-num") + 1], "entry" : entry}
...
def findATAXFR(ata, tag, cam, frame):
	...
	for iteration in range(len(idx)):
		...
		try:
			...
			used = False;	
			try:
                        	if ((str(item) in ataDebug["xfr"][f].keys()) and (ataDebug["xfr"][f][str(item)] != "false alarm...")) or str(item) in gtDebug["xfr"][frame].values():
					used = True;
                        except KeyError:
				pass
	
			if used == False and item[0] == "xfr" and item[item.index("camera-num") + 1] == cam:
				if item[item.index("type") + 1] == "from" and priorFrameObjects(tag, item[item.index("dvi-id") + 1]): # and item[item.index("dvi-id") + 1] != tag:
...
def match_xfr_event(gt, ata, gtFrame, event):
	...
	try:
                if str(event) in ataDebug["xfr"][f].keys() and ataDebug["xfr"][f][str(event)] != "false alarm...":
                        used = True;
        except KeyError:
		pass
	
	frame, xfr_idx, mismatch = findATAXFR(ata, event[event.index("dvi-id") + 1], event[event.index("camera-num") + 1], gtFrame);
...
def xfr_prompt(gt, ata, idx):
				...
				for event in gt[idx]:
					...
					if event[event.index("id") + 1]  == dviTag:
						...
						print "matching... " + str(event[0])
                                		...
                                		if match_loc_event(ata, event, idx, event[event.index("type") + 1]):
        ...
        try:
			...
			if entry[0] == "xfr":
                                try:
                                        if str(entry) in ataDebug["xfr"][idx].keys():
                                                continue
                                except KeyError:
					pass
...
def prompt(gt, ata, idx):
	try:
		...
		for entry in gt[idx]:
			...
			if entry[0] == "loc":
				...
				if match_loc_event(ata, entry, idx, entry[entry.index("type") + 1]):
					if entry[entry.index("type") + 1] == "dvi":
						results["# PD (DVI)"] = results["# PD (DVI)"] + 1;
					elif entry[entry.index("type") + 1] == "pax":
	...
	try:
		...
		for entry in ata[idx]:
			...
			if entry[0] == "loc":
				try:
					if str(entry) in ataDebug["pax"][idx].keys() or str(entry) in gtDebug["pax"][idx].values():
						continue
				except:
					pass
				try:
					if str(entry) in ataDebug["dvi"][idx].keys() or str(entry) in gtDebug["dvi"][idx].values():
                                        	continue
				except:
					pass
				...
				try:
					if str(entry) in ataDebug["xfr"][idx].keys(): # or str(entry) in gtDebug["xfr"][idx].values():
						continue
				except KeyError:
					pass
				ataDebug["xfr"][idx][str(entry)] = "false alarm..."
		...
		try:
			...
			for item in ata[str(i).zfill(5)]:
				if item[0] == "loc" and item[2] == "pax":
					ataDebug["pax"][str(i).zfill(5)][str(item)] = "not considered..."
				elif item[0] == "loc" and item[2] == "dvi":
					ataDebug["dvi"][str(i).zfill(5)][str(item)] = "not considered..."
				elif item[0] =="xfr":
					ataDebug["xfr"][str(i).zfill(5)][str(item)] = "not considered..."
		except KeyError:
			pass
	...
	for t in ["pax", "dvi", "xfr"]:
		try:
			...
			for item in gtDebug[t][str(i).zfill(5)]:
				...
				log(GtoutFile, "ATA: " + str(gtDebug[t][str(i).zfill(5)][item]) + "\n")
		except KeyError:
			pass
		try:
			ataDebug[t][str(i).zfill(5)]
			for item in ataDebug[t][str(i).zfill(5)]:
				log(AtaoutFile, str(i).zfill(5) + ":")
                                ...
                                log(AtaoutFile, "GT:  " + str(ataDebug[t][str(i).zfill(5)][item]) + "\n")
		except KeyError:
			pass
...
for f in ataDebug["xfr"]:
	for item in ataDebug["xfr"][f]:
		if ataDebug["xfr"][f][item] == "false alarm...":
			...
			arr_item = item.replace("[", '').replace("]", '').replace(",",'').replace("\'", '').split(" ")
			# annotateText(str(af).zfill(5), arr_item, "fa", f)
			results["# PFA (XFR)"] = results["# PFA (XFR)"] + 1

# NOTE: Warning for when id's are reused
if WARNING:
	print "WARNING! WARNING! WARNING! IDs were reused within the same frame by the algorithm!!!"
